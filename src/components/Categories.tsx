...
function Categories({ data }: Props) {
  const [activeCategory, setActiveCategory] = useState('productionDelivery')

  const categories = [
    { id: 'productionDelivery', label: 'Production & Delivery', data: data.categories.productionDelivery },
  ...
  // Type guard to check if activeData is a Category (not ProductionDeliveryCategory)
  const isCategory = (data: any): data is Category => {
    return data && 'keyPoints' in data
  }
  ...
  return (
    <div>
      ...
      {/* Category Content */}
      {activeCategory === 'productionDelivery' ? (
        ...
        <div>
          ...
          {/* Key Points */}
          {isCategory(activeData) && activeData.keyPoints && activeData.keyPoints.length > 0 && (
            <div className="mb-6">
          ...
          {/* Timeline */}
          {isCategory(activeData) && activeData.timeline && activeData.timeline.length > 0 && (
            <div>
