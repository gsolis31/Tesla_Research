...
# chains: lint-curated → merge → url-cache → archive → python-validate → zod-validate → build
```

### Deployment

```bash
git add data/tesla-tracking-data.json research/findings/YYYY-MM-DD.json research/findings/url-cache.json research/logs/YYYY-MM-DD
git commit -m "Update: …"
...
| `npm test:lint` | Research lint gold evals | `pytest tests/test_lint_findings.py` |
| `python3 scripts/lint_findings.py --raw --write-logs` | Lint researcher output | Writes `research/logs/YYYY-MM-DD/` |
...
| `python3 -m pytest` | Full pytest suite | merge, spawn configs, lint evals |
...
1. **Agents** (`.claude/agents/`)
   - `tesla-researcher.md` — Researches one category; outputs `research/raw/findings-{category}.json` (must include `searchLog`)
   - `tesla-coverage-scout.md` — Independent news sweep; writes `research/logs/YYYY-MM-DD/coverage.json` (no findings)
...
4. **Merge & Validation** (`scripts/`)
   ...
   - `lint_findings.py` — Deterministic lint of raw + curated findings; writes `research/logs/`
   - `finalize_update.py` — One command: lint-curated → merge → cache → archive → validate → build
...
│   │   ├── curator-report-YYYY-MM-DD.md
│   │   ├── url-cache.json            # Seen URLs (deduplication)
│   │   ├── schema.json               # Curated findings schema
│   │   └── raw-schema.json           # Researcher output schema (searchLog)
...
│   │   ├── tesla-coverage-scout.md   # Independent recall check (not a writer)
...
│   ├── test_merge_findings.py        # Merge logic regressions
│   ├── test_lint_findings.py         # Research lint gold evals
│   ├── evals/                        # Frozen raw/curated fixtures
│   └── README.md
...
2. Research (Parallel agents via /tesla-update-v2 skill)
   ...
   ... (9 categories)
   tesla-coverage-scout (with batch 3) → research/logs/YYYY-MM-DD/coverage.json
   ...
   - Outputs findings-{category}.json (~2KB) including `searchLog`

3. Lint raw
   python3 scripts/lint_findings.py --raw --date 2026-07-08 --write-logs
   → research/logs/2026-07-08/{lint-raw.json, run.json, {category}.json}

4. Curation
   tesla-curator:
   - Loads 9 × findings-{category}.json plus curator-config (slim last week + `seenUrls` + lintReport)
   - Drops lint-raw errors (registration-as-fleet, ownership trespass, stale dates)
...
5. Lint curated + Merge
   python3 scripts/lint_findings.py --curated research/findings/2026-07-08.json --write-logs
...
npm test:lint         # Research lint gold evals
```
