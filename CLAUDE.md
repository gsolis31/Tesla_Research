...
3. **Pipeline Orchestrators** (Skills in `.claude/skills/`)
   - `/tesla-update-v2` (PREFERRED) — Orchestrates batched parallel research + merge + build + commit
   - `/tesla-update` (DEPRECATED) — Old sequential workflow
   - `/tesla-research-parallel` — (NOT ACTIVE) Parallel research mode
...
5. **Research Directory** (`research/`)
   ```
   research/
   ├── configs/          # Generated (spawn_researcher.py, spawn_curator.py)
   ├── raw/              # Per-category findings (researcher agent outputs)
   └── findings/         # Curated weekly packages
...
├── research/
│   ├── configs/                      # Generated configs (spawn_researcher.py, spawn_curator.py)
│   │   ├── research-config-*.json
│   │   └── curator-config.json
│   ├── raw/                          # Per-category findings (researcher outputs)
...
│   ├── validate-zod-schema.ts        # Zod validation (CI/CD check)
...
│   └── parallel_research.py          # (Not active) parallel coordinator
├── .claude/
│   ├── agents/
│   │   ├── tesla-researcher.md       # Research agent prompt
│   │   └── tesla-curator.md          # Curation agent prompt
│   └── skills/
│       ├── tesla-update/             # V1 skill (deprecated)
│       ├── tesla-update-v2/          # Batched skill (PREFERRED)
│       └── tesla-research-parallel/  # Parallel skill (not active)
├── .github/
│   └── workflows/
...
│   ├── PARALLEL_RESEARCH.md          # Grok #3: batched agents (not active)
│   ├── AUTOMATION_SETUP.md
│   ├── CURATION_SUMMARY.md
│   ├── DEAD_FILES.md                 # Obsolete workflows
...
| `docs/DEAD_FILES.md` | Obsolete workflows, cleanup notes |
